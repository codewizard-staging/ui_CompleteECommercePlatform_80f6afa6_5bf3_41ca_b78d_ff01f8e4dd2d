import AnalyticsJsonConfig from "config/analytics_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "analytics", expand: "", exclude: [], func: Support.AddOrUpdateAnalytics }
]

const FetchAnalyticsInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(AnalyticsJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of AnalyticsJsonConfig[elm]) {
...
const FetchAnalyticsDetails = async (analyticsId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (analyticsId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetAnalyticsSingle(analyticsId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetAnalyticsSingle(analyticsId);
            }
            
            if (rslt.status) {

                const analytics = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? analytics : analytics[source];
...
const Extract = async (analyticsId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchAnalyticsInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(analyticsId)) {
                    await FetchAnalyticsDetails(analyticsId, enums).then(({ row, backRow }) => {
