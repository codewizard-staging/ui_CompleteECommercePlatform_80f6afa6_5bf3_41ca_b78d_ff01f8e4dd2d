...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, analyticsId, numfields;
        const mapItems = MapItems;

        let analytics = row['analytics'];

        numfields = Helper.GetAllNumberFields(analytics);
        if (numfields.length > 0) Helper.UpdateNumberFields(analytics, numfields);


        // Add Or Update Analytics
        rslt = await Support.AddOrUpdateAnalytics(analytics, []);
        if (rslt.status) {
            analyticsId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Analytics and navigation entity if it is deos not exist
            let navItem = analytics.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "AnalyticsId", value: parseInt(analyticsId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateAnalytics(data);
        ...
        global.AlertPopup("success", "Analytics is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
