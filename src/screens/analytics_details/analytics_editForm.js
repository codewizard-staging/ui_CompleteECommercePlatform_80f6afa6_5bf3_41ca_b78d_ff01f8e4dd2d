...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], analyticsId, analytics, numfields, mapItem;

        analytics = row['analytics'];
        analyticsId = row['analytics'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) analyticsId = rslt.id;
                            
                            const mapPropKey = analytics.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "AnalyticsId", value: parseInt(analyticsId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateAnalytics(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'analytics');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Analytics is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
