...
const Component = (props) => {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetUserSingle({ UserId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Users/edit/${id}`;
        if (type === 'view') _route = `/Users/view/${id}`;
