...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetWishlistsCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetWishlistsMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetWishlistSingle({ WishlistId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Wishlists/edit/${id}`;
        if (type === 'view') _route = `/Wishlists/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Wishlists/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'WishlistId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
