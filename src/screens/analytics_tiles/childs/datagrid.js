        ...
        <>
            <GridContainer>
                {rows && rows.map((x, index) => (
                    <React.Fragment key={`${x.AnalyticsId}_${index}`} >
                        <CardItem keyid={x.AnalyticsId} row={x} title={x.Sales}  imgsrc={x.AnalyticsReportDateData}  width={300}
                            footerItems={[]} description={x.Traffic} onActionClicked={OnActionClicked}>
