...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetAnalyticsCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetAnalyticsSingle({ AnalyticsId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Analytics/edit/${id}`;
        if (type === 'view') _route = `/Analytics/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Analytics/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
