...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, customerSupportId, numfields;
        const mapItems = MapItems;

        let customersupport = row['customersupport'];

        numfields = Helper.GetAllNumberFields(customersupport);
        if (numfields.length > 0) Helper.UpdateNumberFields(customersupport, numfields);


        // Add Or Update CustomerSupport
        rslt = await Support.AddOrUpdateCustomerSupport(customersupport, []);
        if (rslt.status) {
            customerSupportId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the CustomerSupport and navigation entity if it is deos not exist
            let navItem = customersupport.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "TicketId", value: parseInt(customerSupportId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateCustomerSupport(data);
        ...
        global.AlertPopup("success", "CustomerSupport is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
