import CustomerSupportJsonConfig from "config/customersupport_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "customersupport", expand: "", exclude: [], func: Support.AddOrUpdateCustomerSupport }
]

const FetchCustomerSupportInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(CustomerSupportJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of CustomerSupportJsonConfig[elm]) {
...
const FetchCustomerSupportDetails = async (customersupportId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (customersupportId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetCustomerSupportSingle(customersupportId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetCustomerSupportSingle(customersupportId);
            }
            
            if (rslt.status) {

                const customersupport = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? customersupport : customersupport[source];
...
const Extract = async (customersupportId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchCustomerSupportInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(customersupportId)) {
                    await FetchCustomerSupportDetails(customersupportId, enums).then(({ row, backRow }) => {
