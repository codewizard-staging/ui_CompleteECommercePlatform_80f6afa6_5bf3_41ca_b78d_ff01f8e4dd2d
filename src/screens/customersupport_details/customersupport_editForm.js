...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], customerSupportId, customersupport, numfields, mapItem;

        customersupport = row['customersupport'];
        customerSupportId = row['customersupport'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) customerSupportId = rslt.id;
                            
                            const mapPropKey = customersupport.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "TicketId", value: parseInt(customerSupportId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateCustomerSupport(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'customersupport');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "CustomerSupport is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
