...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetShipmentsCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetShipmentSingle({ ShipmentId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Shipments/edit/${id}`;
        if (type === 'view') _route = `/Shipments/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Shipments/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
