...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetProductsMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetProductSingle({ ProductId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    return (
        ...
        <>
            <Container {...props}>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'ProductId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
