...
const columns = [
    { headerName: "OrderDate", field: "OrderDate", flex: 1 },
    { headerName: "Status", field: "Status", flex: 1 },
    { headerName: "TotalAmount", field: "TotalAmount", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetOrdersMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetOrderSingle({ OrderId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Orders/edit/${id}`;
        if (type === 'view') _route = `/Orders/view/${id}`;
