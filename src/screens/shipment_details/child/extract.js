import ShipmentJsonConfig from "config/shipment_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "shipment", expand: "", exclude: [], func: Support.AddOrUpdateShipment }
]

const FetchShipmentInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(ShipmentJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of ShipmentJsonConfig[elm]) {
...
const FetchShipmentDetails = async (shipmentId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (shipmentId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetShipmentSingle(shipmentId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetShipmentSingle(shipmentId);
            }
            
            if (rslt.status) {

                const shipment = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? shipment : shipment[source];
...
const Extract = async (shipmentId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchShipmentInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(shipmentId)) {
                    await FetchShipmentDetails(shipmentId, enums).then(({ row, backRow }) => {
