...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, shipmentId, numfields;
        const mapItems = MapItems;

        let shipment = row['shipment'];

        numfields = Helper.GetAllNumberFields(shipment);
        if (numfields.length > 0) Helper.UpdateNumberFields(shipment, numfields);


        // Add Or Update Shipment
        rslt = await Support.AddOrUpdateShipment(shipment, []);
        if (rslt.status) {
            shipmentId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Shipment and navigation entity if it is deos not exist
            let navItem = shipment.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "ShipmentId", value: parseInt(shipmentId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateShipment(data);
        ...
        global.AlertPopup("success", "Shipment is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
