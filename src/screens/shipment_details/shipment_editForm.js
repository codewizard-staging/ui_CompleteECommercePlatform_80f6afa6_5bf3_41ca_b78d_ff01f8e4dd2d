...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], shipmentId, shipment, numfields, mapItem;

        shipment = row['shipment'];
        shipmentId = row['shipment'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) shipmentId = rslt.id;
                            
                            const mapPropKey = shipment.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "ShipmentId", value: parseInt(shipmentId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateShipment(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'shipment');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Shipment is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
