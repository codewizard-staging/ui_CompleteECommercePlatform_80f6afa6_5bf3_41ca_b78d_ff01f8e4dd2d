...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetDiscountsCount(query)
            .then(async (res) => {
                if (res.status) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetDiscountSingle({ DiscountId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Discounts/edit/${id}`;
        if (type === 'view') _route = `/Discounts/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Discounts/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
