...
const Component = (props) => {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetCartSingle({ CartId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Carts/edit/${id}`;
        if (type === 'view') _route = `/Carts/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Carts/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'CartId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
