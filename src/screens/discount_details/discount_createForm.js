...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, discountId, numfields;
        const mapItems = MapItems;

        let discount = row['discount'];

        numfields = Helper.GetAllNumberFields(discount);
        if (numfields.length > 0) Helper.UpdateNumberFields(discount, numfields);


        // Add Or Update Discount
        rslt = await Support.AddOrUpdateDiscount(discount, []);
        if (rslt.status) {
            discountId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Discount and navigation entity if it is deos not exist
            let navItem = discount.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "DiscountId", value: parseInt(discountId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateDiscount(data);
        ...
        global.AlertPopup("success", "Discount is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
