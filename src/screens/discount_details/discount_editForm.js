...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], discountId, discount, numfields, mapItem;

        discount = row['discount'];
        discountId = row['discount'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) discountId = rslt.id;
                            
                            const mapPropKey = discount.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "DiscountId", value: parseInt(discountId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateDiscount(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'discount');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Discount is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
