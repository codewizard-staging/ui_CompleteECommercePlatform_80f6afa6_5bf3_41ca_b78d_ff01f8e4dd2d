...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, cartId, numfields;
        const mapItems = MapItems;

        let cart = row['cart'];

        numfields = Helper.GetAllNumberFields(cart);
        if (numfields.length > 0) Helper.UpdateNumberFields(cart, numfields);


        // Add Or Update Cart
        rslt = await Support.AddOrUpdateCart(cart, []);
        if (rslt.status) {
            cartId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Cart and navigation entity if it is deos not exist
            let navItem = cart.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "CartId", value: parseInt(cartId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateCart(data);
        ...
        global.AlertPopup("success", "Cart is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
