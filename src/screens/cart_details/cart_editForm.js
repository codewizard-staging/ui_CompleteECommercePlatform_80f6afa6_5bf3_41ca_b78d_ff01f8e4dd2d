...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], cartId, cart, numfields, mapItem;

        cart = row['cart'];
        cartId = row['cart'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) cartId = rslt.id;
                            
                            const mapPropKey = cart.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "CartId", value: parseInt(cartId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateCart(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'cart');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Cart is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
