import CartJsonConfig from "config/cart_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "cart", expand: "", exclude: [], func: Support.AddOrUpdateCart }
]

const FetchCartInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(CartJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of CartJsonConfig[elm]) {
...
const FetchCartDetails = async (cartId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (cartId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetCartSingle(cartId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetCartSingle(cartId);
            }
            
            if (rslt.status) {

                const cart = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? cart : cart[source];
...
const Extract = async (cartId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchCartInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(cartId)) {
                    await FetchCartDetails(cartId, enums).then(({ row, backRow }) => {
