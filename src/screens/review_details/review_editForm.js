...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], reviewId, review, numfields, mapItem;

        review = row['review'];
        reviewId = row['review'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) reviewId = rslt.id;
                            
                            const mapPropKey = review.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "ReviewId", value: parseInt(reviewId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateReview(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'review');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Review is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
