...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, reviewId, numfields;
        const mapItems = MapItems;

        let review = row['review'];

        numfields = Helper.GetAllNumberFields(review);
        if (numfields.length > 0) Helper.UpdateNumberFields(review, numfields);


        // Add Or Update Review
        rslt = await Support.AddOrUpdateReview(review, []);
        if (rslt.status) {
            reviewId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Review and navigation entity if it is deos not exist
            let navItem = review.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "ReviewId", value: parseInt(reviewId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateReview(data);
        ...
        global.AlertPopup("success", "Review is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
