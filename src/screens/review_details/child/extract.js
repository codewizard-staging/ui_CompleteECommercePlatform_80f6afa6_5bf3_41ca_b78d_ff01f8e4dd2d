import ReviewJsonConfig from "config/review_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "review", expand: "", exclude: [], func: Support.AddOrUpdateReview }
]

const FetchReviewInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(ReviewJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of ReviewJsonConfig[elm]) {
...
const FetchReviewDetails = async (reviewId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (reviewId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetReviewSingle(reviewId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetReviewSingle(reviewId);
            }
            
            if (rslt.status) {

                const review = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? review : review[source];
...
const Extract = async (reviewId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchReviewInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(reviewId)) {
                    await FetchReviewDetails(reviewId, enums).then(({ row, backRow }) => {
