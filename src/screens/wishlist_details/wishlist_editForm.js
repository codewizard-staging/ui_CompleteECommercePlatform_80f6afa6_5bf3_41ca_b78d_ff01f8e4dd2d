...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], wishlistId, wishlist, numfields, mapItem;

        wishlist = row['wishlist'];
        wishlistId = row['wishlist'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) wishlistId = rslt.id;
                            
                            const mapPropKey = wishlist.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "WishlistId", value: parseInt(wishlistId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateWishlist(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'wishlist');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "Wishlist is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
