...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, wishlistId, numfields;
        const mapItems = MapItems;

        let wishlist = row['wishlist'];

        numfields = Helper.GetAllNumberFields(wishlist);
        if (numfields.length > 0) Helper.UpdateNumberFields(wishlist, numfields);


        // Add Or Update Wishlist
        rslt = await Support.AddOrUpdateWishlist(wishlist, []);
        if (rslt.status) {
            wishlistId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the Wishlist and navigation entity if it is deos not exist
            let navItem = wishlist.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "WishlistId", value: parseInt(wishlistId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateWishlist(data);
        ...
        global.AlertPopup("success", "Wishlist is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
