import WishlistJsonConfig from "config/wishlist_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "wishlist", expand: "", exclude: [], func: Support.AddOrUpdateWishlist }
]

const FetchWishlistInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(WishlistJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of WishlistJsonConfig[elm]) {
...
const FetchWishlistDetails = async (wishlistId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (wishlistId) {
            ...
            if ($expand.length > 0) {
                rslt = await Api.GetWishlistSingle(wishlistId, `$expand=${$expand}`);
            } else {
                rslt = await Api.GetWishlistSingle(wishlistId);
            }
            
            if (rslt.status) {

                const wishlist = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? wishlist : wishlist[source];
...
const Extract = async (wishlistId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchWishlistInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(wishlistId)) {
                    await FetchWishlistDetails(wishlistId, enums).then(({ row, backRow }) => {
